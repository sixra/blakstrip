<script lang="ts">
  ...
  const sevClass = (s: FindingSeverity): string => (s === 'high' ? 'bg-danger' : 'bg-warning');
</script>

{#if findings.length > 0}
  <h2 class="text-warning text-sm font-semibold">{heading(findings.length)}</h2>
  <ul class="mt-2 space-y-1">
    {#each findings as f (f.id)}
      <li class="text-muted flex items-start gap-2 text-sm">
        ...
        <span
          ><span class="sr-only">{f.severity === 'high' ? 'High' : 'Medium'} severity: </span><span
            class="text-ink font-medium">{f.title}:</span
          >
          {f.detail}</span
        >
      </li>
